...
use crate::{CameraError, CameraFrame, CameraTrait, FrameCallback};
...
pub struct SVBonyCamera {
    ...
    threadid: Option<Arc<std::thread::JoinHandle<()>>>,
}
...
impl CameraTrait for SVBonyCamera {
    fn name(&self) -> String {
    ...
    fn start(&mut self) -> Result<(), CameraError> {
        ...
        let threadid = std::thread::spawn(move || {
            cam.run().unwrap_or_else(|e| {
                eprintln!("Camera thread error: {}", e);
            });
        });
        self.threadid = Some(Arc::new(threadid));
        Ok(())
    }

    fn stop(&mut self) -> Result<(), CameraError> {
        SVBonyCamera::stop_capture(self).map_err(|e| match e {
            SVBError::InvalidId => CameraError::Connection,
            _ => CameraError::Other(format!("Failed to stop camera: {}", e)),
        })?;
        if let Some(tid) = self.threadid.take() {
            Arc::try_unwrap(tid)
                .map_err(|_| CameraError::Other("Failed to join camera thread".to_string()))?
                .join()
                .map_err(|_| CameraError::Other("Failed to join camera thread".to_string()))?;
        }
        Ok(())
...
impl SVBonyCamera {
    ...
    pub fn stop_capture(&mut self) -> SVBResult<()> {
...
mod test {
    ...
    fn test_in_thread() {
        ...
        cam.set_frame_callback(move |frame: &CameraFrame| -> Result<(), CameraError> {
            println!(
                "frame: exp={} ts={} size={}x{}",
                frame.exposure,
